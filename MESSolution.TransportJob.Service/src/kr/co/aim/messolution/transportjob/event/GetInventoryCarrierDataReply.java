...
public class GetInventoryCarrierDataReply extends AsyncHandler {
	...
	{
		EventInfo eventInfo = EventInfoUtil.makeEventInfo("GetInventoryCarrierDataReply", getEventUser(), getEventComment(), "", "");
		eventInfo.setEventTimeKey(TimeUtils.getEventTimeKeyFromTimestamp(eventInfo.getEventTime()));

		String machineName = SMessageUtil.getBodyItemValue(doc, "MACHINENAME", true);
		List<Element> carrierList = SMessageUtil.getBodySequenceItemList(doc, "CARRIERLIST", true);
		...
		for (Element carrierE : carrierList)
		{
			...
			if (StringUtils.equals(GenericServiceProxy.getConstantMap().MCS_POSITIONTYPE_PORT, currentPositionType))
			{
			...
			else if (StringUtils.equals(GenericServiceProxy.getConstantMap().MCS_POSITIONTYPE_SHELF, currentPositionType))
			{
		...
		// Location Update for Not reported CST
		for (Map<String, Object> carrierInMachine : carrierListInMachine)
		{
			String carrierName = carrierInMachine.get("DURABLENAME").toString();
			Durable durableData = MESDurableServiceProxy.getDurableServiceUtil().getDurableData(carrierName);
			Map<String, String> durableUdfs = new HashMap<String, String>();

...
public class QueueTimer implements Job, InitializingBean
{	
	...
	public void monitorQueueTime() throws CustomException
	{
		...
		sql.append("           FROM CT_PRODUCTQUEUETIME Q ");
		sql.append("          WHERE 1 = 1 ");
		sql.append("            AND Q.ENTERTIME IS NOT NULL ");
		sql.append("            AND Q.QUEUETIMESTATE NOT IN ('Resolved', 'Exited', 'Interlocked')) ");
		...
		EventInfo eventInfo = EventInfoUtil.makeEventInfo("", "MES", "Q-time monitoring service", null, null);
		...
		for (Map<String, Object> row : result)
		{
			...
			if(QTimeData.getQueueTimeState().equals("Exited"))
			{
				continue;
			}
			//end
		...
		try
		{
			...
			GenericServiceProxy.getTxDataSourceManager().beginTransaction();
			{
				...
				if (toQueueTimeState.equals(GenericServiceProxy.getConstantMap().QTIME_STATE_OVER) && !QTimeData.getQueueTimeState().equals(toQueueTimeState) 
						&& (QTimeData.getQueueTimeState().equals("Entered") || QTimeData.getQueueTimeState().equals("Warning")))
				{
					ExtendedObjectProxy.getProductQTimeService().lockQTime(eventInfo, productName, factoryName, processFlowName,
						...
						&& QTimeData.getQueueTimeState().equals("Entered"))
				{

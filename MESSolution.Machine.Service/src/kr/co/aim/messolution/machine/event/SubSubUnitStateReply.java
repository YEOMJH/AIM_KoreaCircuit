...
public class SubSubUnitStateReply extends AsyncHandler {
	...
	public void doWorks(Document doc) throws CustomException
	{
		...
		for (Element unitElement : unitList)
		{
			...
			{
				List<Element> subSubUnitList = SMessageUtil.getSubSequenceItemList(unitElement, "SUBSUBUNITLIST", false);

				for (Element subSubUnitElement : subSubUnitList)
				{
					String machineName = SMessageUtil.getChildText(subUnitElement, "SUBSUBUNITNAME", true);
					String machineStateName = SMessageUtil.getChildText(subUnitElement, "SUBSUBUNITSTATENAME", true);

					try
					{
						Machine machineData = MESMachineServiceProxy.getMachineInfoUtil().getMachineData(machineName);

						if (StringUtil.equals(machineData.getMachineStateName(), "DOWN")
								&& !(StringUtil.equals(machineData.getReasonCode(), "") || StringUtil.equals(machineData.getReasonCodeType(), "")))

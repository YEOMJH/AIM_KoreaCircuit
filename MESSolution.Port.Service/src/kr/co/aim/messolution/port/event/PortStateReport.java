...
public class PortStateReport extends AsyncHandler {
	...
	public void doWorks(Document doc) throws CustomException
	{
		...
		for (Element elePort : portList)
		{
			try
			{
				...
				if (StringUtils.equals(sPortStateName, GenericServiceProxy.getConstantMap().Port_EMPTY) || StringUtils.equals(sPortStateName, GenericServiceProxy.getConstantMap().Port_FULL))
				{
					newPortStateName = GenericServiceProxy.getConstantMap().Port_UP;
				}
				else
				{
					newPortStateName = sPortStateName;
				}

				MakePortStateByStateInfo makePortStateByStateInfo = MESPortServiceProxy.getPortInfoUtil().makePortStateByStateInfo(portData, newPortStateName);
				...
				// change port type
				if (StringUtils.equalsIgnoreCase(machineSpec.getMachineGroupName(), GenericServiceProxy.getConstantMap().MachineGroup_Sorter) && StringUtils.equals(sPortType, "PB"))
				{
					...
					transitionInfo.getUdfs().put("PORTTYPE", "PS");

...
public class ReleaseHoldShield extends SyncHandler {
	...
	public Object doWorks(Document doc) throws CustomException
	{
		...
		EventInfo eventInfo = EventInfoUtil.makeEventInfo("ReleaseHoldShield", this.getEventUser(), this.getEventComment(), "", "");
		...
		for (Element shield : shieldList)
		{
			...
			CommonValidation.checkShieldLotHoldStateY(shieldLotData);

			if (StringUtil.isEmpty(carrierName))
			{
				carrierName = shieldLotData.getCarrierName();
			}

			List<Object> updateBindList = new ArrayList<Object>();
			updateBindList.add("N");
			updateBindList.add("");
			updateBindList.add("");
	...
	private void updateShieldLot(List<Object[]> updateArgsList) throws CustomException
	{
		...
		sql.append(" SET LOTHOLDSTATE = ? , REASONCODETYPE = ? , REASONCODE = ? , CARRIERNAME = ? , LASTEVENTCOMMENT = ? , LASTEVENTNAME = ? , LASTEVENTUSER = ? , LASTEVENTTIME = ? , LASTEVENTTIMEKEY = ? ");

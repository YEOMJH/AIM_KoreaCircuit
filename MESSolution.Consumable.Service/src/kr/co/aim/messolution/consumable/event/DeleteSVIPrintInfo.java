...
public class DeleteSVIPrintInfo extends SyncHandler{

	public Object doWorks(Document doc) throws CustomException
	{
		
		List<Element> MACHINELIST = SMessageUtil.getBodySequenceItemList(doc, "DELETESETPRINTINFOLIST", true);

		for (Element MACHINE : MACHINELIST)
		{
				String timeKey = SMessageUtil.getChildText(MACHINE, "TIMEKEY", true);

				EventInfo eventInfo = EventInfoUtil.makeEventInfo("DeleteSVIPrintInfo", getEventUser(), getEventComment(), "", "");
				...
				if(!sqlResult6.isEmpty()){
					throw new CustomException("DEFECTCODE-0006", pickInfo.getTimeKey(),"1");
				}
				
				ExtendedObjectProxy.getSVIPickInfoService().remove(eventInfo, pickInfo);
		}
		return doc;
	}
}

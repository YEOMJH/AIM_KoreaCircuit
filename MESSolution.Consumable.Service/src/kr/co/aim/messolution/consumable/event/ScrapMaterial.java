...
public class ScrapMaterial extends SyncHandler {
	...
	public Object doWorks(Document doc) throws CustomException
	{
		...
		EventInfo eventInfo = EventInfoUtil.makeEventInfo("ScrapMaterial", getEventUser(), getEventComment(), null, null);
		...
		for (Element element : materialElementList)
		{
			...
			if (materialKind.equals("Consumable"))
			{
				...
				if (!GenericServiceProxy.getConstantMap().Cons_Available.equals(consumableData.getConsumableState()))
			...
			else if (materialKind.equals("Durable"))
			{
				...
				// Durable Update
				kr.co.aim.greentrack.durable.management.info.MakeNotAvailableInfo makeNotAvailableInfo = new kr.co.aim.greentrack.durable.management.info.MakeNotAvailableInfo();

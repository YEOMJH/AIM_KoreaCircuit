...
public class ChangeMaskCycleCount extends SyncHandler {

	private static Log log = LogFactory.getLog(ChangeMaskCycleCount.class);
	...
	public Object doWorks(Document doc) throws CustomException
	{
		...
		for (Element Mask : MaskList)
		{
			...
			String maskCycleCount = SMessageUtil.getChildText(Mask, "MASKCYCLECOUNT", true);
			

			// Set Mask Info
			EventInfo eventInfo = EventInfoUtil.makeEventInfo("ChangeMaskCycleCount", this.getEventUser(), this.getEventComment(), "", "");
			...
			boolean matches =maskCycleCount.matches("[0-9]+");
			
			if(!matches){
				throw new CustomException("MASK-0093",maskLotData.getMaskLotName());
			}
			
			if(!StringUtil.isBlank(maskCycleCount) && (maskLotData.getMaskCycleCount().intValue()==Integer.valueOf(maskCycleCount))){
				throw new CustomException("MASK-0094",maskLotData.getMaskLotName());
			}else{
				maskLotData.setMaskCycleCount(Integer.valueOf(maskCycleCount));
			}

...
public class ModifyReserveDummyProduct extends SyncHandler {
	...
	public Object doWorks(Document doc) throws CustomException
	{
		...
		EventInfo eventInfo = EventInfoUtil.makeEventInfo("ModifyReserveDummyProduct", getEventUser(), getEventComment(), null, null);
		...
		int maxSeq = ExtendedObjectProxy.getDummyProductReserveService().getMaxSeq(lotName);
		...
		if (operationList.size() > 0)
		{
			for (Element operation : operationList)
			{
				...
				if (Integer.parseInt(seq) > maxSeq)
				{

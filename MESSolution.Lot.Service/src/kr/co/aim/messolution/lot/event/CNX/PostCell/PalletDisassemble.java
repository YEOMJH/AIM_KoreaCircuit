...
public class PalletDisassemble extends SyncHandler {

	private static Log log = LogFactory.getLog(PalletDisassemble.class);
	
	@Override
	public Object doWorks(Document doc) throws CustomException
	{
		String palletName = SMessageUtil.getBodyItemValue(doc, "PROCESSGROUPNAME", true);
		...
		Object[] bindSet = new Object[] { palletName };

		List<ProcessGroup> outerBoxList = ProcessGroupServiceProxy.getProcessGroupService().select(condition, bindSet);

		List<Object[]> updateLotArgList = new ArrayList<Object[]>();
		List<LotHistory> updateLotHistoryList = new ArrayList<LotHistory>();

		for (ProcessGroup boxData : outerBoxList)
		{
			Object[] bindSetInner = new Object[] { boxData.getKey().getProcessGroupName() };

			List<ProcessGroup> innerBoxList = ProcessGroupServiceProxy.getProcessGroupService().select(condition, bindSetInner);

			for (ProcessGroup innerBoxData : innerBoxList)
			{
				List<Lot> lotList = MESLotServiceProxy.getLotServiceUtil().getLotListByProcessGroup(innerBoxData.getKey().getProcessGroupName());
				...
				for (Lot lot : lotList)
				{
					...
					List<Map<String, Object>> operationData = MESLotServiceProxy.getLotServiceUtil().getOperationByDetailOperType(lot.getFactoryName(), lot.getProductSpecName(),
							lot.getProductSpecVersion(), lot.getProcessFlowName(), lot.getProcessFlowVersion(), "Palletize");
		...
		ProcessGroup processGroup = MESProcessGroupServiceProxy.getProcessGroupServiceUtil().getProcessGroupData(palletName);
		ProcessGroup oldProcessGroup = (ProcessGroup) ObjectUtil.copyTo(processGroup);

...
public class NGCancelReceiveLot extends SyncHandler{
	...
	public Object doWorks(Document doc) throws CustomException
	{
		...
		for (Element lot : lotList)
		{
			...
			String oldFactoryName = histUdfs.get("FACTORYNAME");
			...
			{
				throw new CustomException("WO-00005");
			}
			...
			if(StringUtil.isNotEmpty(sapFlag)&&StringUtil.equals(sapFlag, "Y")&&
					StringUtil.in(oldWorkOrderData.getProductRequestType(), "E","P","T")&&
					StringUtil.in(workOrderData.getProductRequestType(), "M","D")&&
					StringUtils.isNotEmpty(oldWorkOrderData.getUdfs().get("SUPERPRODUCTREQUESTNAME")))
			{
                superWO = ExtendedObjectProxy.getSuperProductRequestService().selectByKey(false, new Object[]{oldWorkOrderData.getUdfs().get("SUPERPRODUCTREQUESTNAME")});
				
				String SAPlotGrade = baseLotData.getLotGrade();
				if(SAPlotGrade.length() == 1)
					SAPlotGrade += "0";
				...
				else if(StringUtils.equals(productSpecData.getUdfs().get("PRODUCTSPECTYPE"), "L")
						||StringUtils.equals(productSpecData.getUdfs().get("PRODUCTSPECTYPE"), "T"))
				{
					headInfo.put("WERKS", "5001");
					headInfo.put("LGORT_C","OF01");
				}
				else if(StringUtils.equals(productSpecData.getUdfs().get("PRODUCTSPECTYPE"), "F"))
				{
					headInfo.put("WERKS", "5001");
					headInfo.put("LGORT_C","AF02");
				}
				else if(StringUtils.equals(productSpecData.getUdfs().get("PRODUCTSPECTYPE"), "B")
						||StringUtils.equals(productSpecData.getUdfs().get("PRODUCTSPECTYPE"), "C"))
				{
					headInfo.put("WERKS", "5001");
				...
				bodyInfo.put("CHARG_R", oldWorkOrderData.getUdfs().get("SUPERPRODUCTREQUESTNAME") + SAPlotGrade);
				bodyInfo.put("MATNR", productSpecName.substring(0, productSpecName.length() - 1));
	...
	private Map<String, String> findLotHist(String lotName, String factoryName) throws CustomException
	{
		...
		bindMap.put("EVENTNAME", "NGReceive");
	...
	private List<Map<String, Object>> getSourceLotPriority(String lotName) throws CustomException
	{
		...
		sql.append("  FROM LOTHISTORY ");
		sql.append(" WHERE LOTNAME = :LOTNAME ");
		sql.append("   AND EVENTNAME = 'NGReceive' ");

...
public class TrackInOutForTP extends AsyncHandler {
	@Override
	public void doWorks(Document doc) throws CustomException {
		...
		try
		{
			...
			// TrackIn without history
			doc = this.writeTrackInRequest(doc, lotData.getKey().getLotName(), machineName, portName, machineRecipeName);
			result = InvokeUtils.invokeMethod(InvokeUtils.newInstance(TrackInForTP.class.getName(), null, null), "execute", new Object[] {doc});
			
			// TrackOut without history
			doc = this.writeTrackOutRequest(doc, lotData.getKey().getLotName(), machineName, portName, carrierName,machineRecipeName);
			result = InvokeUtils.invokeMethod(InvokeUtils.newInstance(TrackOutForTP.class.getName(), null, null), "execute", new Object[] {doc});
		}
	...
	{
		SMessageUtil.setHeaderItemValue(doc, "MESSAGENAME", "TrackInForTP");
	...
	private Document writeTrackOutRequest(Document doc, String lotName, String machineName, String portName, String carrierName,String machineRecipeName)
			throws CustomException
	{
		SMessageUtil.setHeaderItemValue(doc, "MESSAGENAME", "TrackOutForTP");
		//SMessageUtil.setHeaderItemValue(doc, "ORIGINALSOURCESUBJECTNAME", GenericServiceProxy.getESBServive().getSendSubject("CNXsvr"));
	...
	private Port searchLoaderPort(String machineName) throws CustomException
	{
		try
		{
			List<Port> result = PortServiceProxy.getPortService().select("machineName = ? AND portType = ?", new Object[] {machineName, "PB"});

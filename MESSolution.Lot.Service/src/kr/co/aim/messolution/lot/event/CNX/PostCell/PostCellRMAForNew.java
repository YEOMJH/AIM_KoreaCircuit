...
public class PostCellRMAForNew extends SyncHandler {
	...
	public Object doWorks(Document doc) throws CustomException
	{
		...
		String lotNameforCheck = SMessageUtil.getBodyItemValue(doc, "LOTNAMEFORCHECK", true);
		String porcessOper="";
		...
		List<Object[]> updateLotArgListForDelete = new ArrayList<Object[]>();
		...
		Lot lotDataForCheck = MESLotServiceProxy.getLotInfoUtil().getLotData(lotNameforCheck);
		Lot oldLotDataForCheck = (Lot) ObjectUtil.copyTo(lotDataForCheck);
		lotDataForCheck.setNodeStack(startNodeStack);

		@SuppressWarnings("rawtypes")
		PFIValueSetter valueSetter = new LotPFIValueSetter(pfi, oldLotDataForCheck, lotDataForCheck );
		...
		{
			String trayID = trayData.getChildText("DURABLENAME");
			String position = trayData.getChildText("POSITION");
			List<Element> lotList = trayData.getChild("LOTLIST").getChildren("LOT");
			int lotQuantity = lotList.size();
			...
			for (Element lot : lotList)
			{
				...
				//Delete old data
				List<Object> lotBindListForDelete = new ArrayList<Object>();
				lotBindListForDelete.add(lotName);
				updateLotArgListForDelete.add(lotBindListForDelete.toArray());
		...
		trayGroupData.setLotQuantity(updateLotArgList.size());
		...
		this.incrementProductRequest(eventInfo, updateLotArgList.size(), workOrderName);
				...
				StringUtils.isNotEmpty(workOrderData.getUdfs().get("SUPERPRODUCTREQUESTNAME")))
		{
			...
			ERPInfo.put("QUANTITY", String.valueOf(updateLotArgList.size()));
			ERPInfo.put("CONSUMEUNIT", superWO.getProductType());
            
			if(superWO.getSubProductionType().equals("SYZLC")||superWO.getSubProductionType().equals("SYFG"))
			{
				...
				ERPInfo.put("FACTORYPOSITION", "PF01");
			}
			

		    ERPInfo.put("BATCHNO", batchNo);			
			ERPInfo.put("PRODUCTQUANTITY", String.valueOf(updateLotArgList.size()));
	...
	private void DeleteLotData(List<Object[]> updateLotArgList) throws CustomException
	{
		try
		{
			String deleteSql = "DELETE FROM LOT WHERE LOTNAME = :LOTNAME ";
			MESLotServiceProxy.getLotServiceUtil().updateBatch(deleteSql, updateLotArgList);
		}
		catch (Exception e)
		{
	...
	private void updateLotData(EventInfo eventInfo, List<Object[]> updateLotArgList, List<LotHistory> updateLotHistoryList) throws CustomException
	{
		...
		sql.append(" CARRIERNAME, POSITION) ");
		sql.append("VALUES  ");
		sql.append("(?, ?, ?, ?, ?, ");
		sql.append(" ?, ?, ?, ?, ?, ");
		sql.append(" ?, ?, ?, ?, ?, ");

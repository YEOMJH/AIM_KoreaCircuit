...
public class ScrapUnpacking extends SyncHandler {

	private static Log log = LogFactory.getLog(ScrapUnpacking.class);
	
	@Override
	public Object doWorks(Document doc) throws CustomException
	{
		String scrapBoxName = SMessageUtil.getBodyItemValue(doc, "PROCESSGROUPNAME", true);
		...
		List<Lot> lotList = MESLotServiceProxy.getLotServiceUtil().getScrapLotListByProcessGroup(scrapBoxName);

		for (Lot lot : lotList)
		{
			Lot oldLot = (Lot)ObjectUtil.copyTo(lot);

			List<Map<String, Object>> operationData = MESLotServiceProxy.getLotServiceUtil().getOperationByDetailOperType(lot.getFactoryName(), lot.getProductSpecName(), lot.getProductSpecVersion(),
					lot.getProcessFlowName(), lot.getProcessFlowVersion(), "ScrapPack");
		...
		ProcessGroup processGroup = MESProcessGroupServiceProxy.getProcessGroupServiceUtil().getProcessGroupData(scrapBoxName);

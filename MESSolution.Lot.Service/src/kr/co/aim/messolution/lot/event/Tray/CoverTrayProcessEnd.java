...
public class CoverTrayProcessEnd extends AsyncHandler 
{
	private static Log log = LogFactory.getLog(CoverTrayProcessEnd.class);
	...
	public void doWorks(Document doc) throws CustomException
	{
		...
		if (CommonUtil.equalsIn(CommonUtil.getValue(portData.getUdfs(), "PORTTYPE"), "PU", "BU") && CommonUtil.getValue(portData.getUdfs(), "PORTTYPE").equals(portType))
		{
			...
			else
			{
				if (panelList != null && panelList.size() > 0)
				{
					//SORTER-0010:(In operation mode = {0},PortType = {1},PortName = {2}) this tray [{3}]cannot contain panels.
					throw new CustomException("SORTER-0010",operationMode,portName,portType,portName,trayName);
				}
			}
			
			Durable oldTrayData = (Durable) ObjectUtil.copyTo(trayData);

			trayData.setLotQuantity(panelList.size());
			trayData.setDurableState(panelList.size() ==0 ? constMap.Dur_Available :constMap.Dur_InUse);
			...
			trayData.getUdfs().put("BCRFLAG", BCRFlag);

...
public class SplitLotForIFI extends SyncHandler {
	...
	public Object doWorks(Document doc) throws CustomException
	{
		...
		String destDurableName = SMessageUtil.getBodyItemValue(doc, "DESTDURABLENAME", true);

		List<Element> destProductList = SMessageUtil.getBodySequenceItemList(doc, "DESTPRODUCTLIST", true);
		List<Element> srcProductList = SMessageUtil.getBodySequenceItemList(doc, "SRCPRODUCTLIST", false);
		...
		EventInfo eventInfo = EventInfoUtil.makeEventInfo("SplitLotForIFI", getEventUser(), getEventComment(), "", "");
		...
		Durable srcDurableData = MESDurableServiceProxy.getDurableServiceUtil().getDurableData(lotData.getCarrierName());
		CommonValidation.CheckDurableHoldState(srcDurableData);

		Durable destDurableData = MESDurableServiceProxy.getDurableServiceUtil().getDurableData(destDurableName);
		...
		for (Element eleProduct : destProductList)
		{
			...
			productP.getUdfs().put("CHANGESHOPLOTNAME", lotData.getKey().getLotName());
			...
			ExtendedObjectProxy.getOriginalProductInfoService().insertOriginalProductInfo(eventInfo, productData);
		}
		...
		try
		{
			destLotData = MESLotServiceProxy.getLotInfoUtil().getLotInfoBydurableName(destDurableName);
		}
		...
		if (srcProductList != null)
		{
			String sourLotGrade = CommonUtil.judgeLotGradeByProductList(srcProductList, "PRODUCTJUDGE");
			MESLotServiceProxy.getLotServiceImpl().updateLotData("LOTGRADE", sourLotGrade, lotName);
		}

		if (destProductList != null)
		{
			String destLotGrade = CommonUtil.judgeLotGradeByProductList(destProductList, "PRODUCTJUDGE");
			MESLotServiceProxy.getLotServiceImpl().updateLotData("LOTGRADE", destLotGrade, destLotData.getKey().getLotName());
		}

		UpdateDestLotData(destLotData, destProductList.size(), lotData, prodList);

		// Split Lot
		TransferProductsToLotInfo transitionInfo = MESLotServiceProxy.getLotInfoUtil().transferProductsToLotInfo(destLotData.getKey().getLotName(), destProductList.size(), productPSequence,
...
*/
		...
		// Mantis - 0000043
		// Assign Carrier
		AssignCarrierInfo AssigncreateInfo = MESLotServiceProxy.getLotInfoUtil().AssignCarrierInfo(destLotData, destDurableName, productPSequence);
		...
		if (CommonUtil.equalsIn(baseData.getUdfs().get("PRODUCTSPECGROUP"), "MQC", "MQCPrepare"))
		{
			...
			createMQC(lotData, destLotData, prodList, destDurableName, eventInfo);
		}

...
public class MergeLotForDummy extends SyncHandler {
	public static Log logger = LogFactory.getLog(MergeLotForDummy.class);
	...
	public Object doWorks(Document doc) throws CustomException
	{
		...
		String srcDurableName = SMessageUtil.getBodyItemValue(doc, "SRC_DURABLENAME", true);
		List<Element> srcProductList = SMessageUtil.getBodySequenceItemList(doc, "SRC_PRODUCTLIST", true);

		Lot destLotData = MESLotServiceProxy.getLotInfoUtil().getLotData(destLotName);
		...
		// validation
		CommonValidation.checkLotState(destLotData);
		CommonValidation.checkLotProcessState(destLotData);
		CommonValidation.checkLotHoldState(destLotData);
		CommonValidation.checkJobDownFlag(destLotData);
		CommonValidation.checkOriginalProduct(destLotData);

		if (!destLotData.getProcessGroupName().isEmpty())
		...
		List<Element> eleDestiProductList = SMessageUtil.getSubSequenceItemList(eleBody, "DESTINATIONPRODUCTLIST", true);

		List<String> prodList = new ArrayList<String>();

		Durable sourceDurableData = MESDurableServiceProxy.getDurableServiceUtil().getDurableData(srcDurableName);

		CommonValidation.CheckDurableHoldState(sourceDurableData);

		Lot sourceLotData = MESLotServiceProxy.getLotInfoUtil().getLotInfoBydurableName(srcDurableName);
		CommonValidation.checkJobDownFlag(sourceLotData);

		List<ProductP> productPSequence = setproductPSequence(eventInfo, srcProductList, sourceLotData, destLotData);
		
		ProductRequest productRequestData = MESWorkOrderServiceProxy.getProductRequestServiceUtil().getProductRequestData(sourceLotData.getProductRequestName());
		if (productRequestData.getPlanQuantity() < productRequestData.getReleasedQuantity() + productPSequence.size())
		{
			throw new CustomException("DUMMY-0001", productRequestData.getKey().getProductRequestName(), productRequestData.getPlanQuantity(), productRequestData.getReleasedQuantity(),
					productPSequence.size());
		}
		
		for (Element eleProduct : srcProductList)
		{
		...
		try
		{
			List<Element> eleSrcProductList = SMessageUtil.getSubSequenceItemList(eleBody, srcDurableName, false);
		...
		LotServiceProxy.getLotService().deassignProducts(sourceLotData.getKey(), eventInfo, deassignProductsInfo);
		...
		for (ProductP prodP : productPSequence)
		{
			...
			kr.co.aim.greentrack.product.management.info.ChangeSpecInfo changeSpecInfo = MESProductServiceProxy.getProductInfoUtil().changeSpecInfo(productData, productData.getAreaName(),
					...
					specData.getSubProductUnitQuantity1(), specData.getSubProductUnitQuantity2(), udfs);
		...
		LotServiceProxy.getLotService().assignProducts(destLotData.getKey(), eventInfo, assignProductsInfo);

		// Set ProductRequest Quantity
		setProductRequestQuantity(eventInfo, sourceLotData, destLotData, productPSequence.size());

		srcLotList.add(sourceLotData.getKey().getLotName());

		for (Element eleSourceProduct : srcProductList)
		{
		...
		if (CommonUtil.equalsIn(baseData.getUdfs().get("PRODUCTSPECGROUP"), "MQC", "MQCPrepare"))
		{
			...
			ExistMQCJobDestLot = ExistMQCJobLot(destLotData);
			...
			if (ExistMQCJobSrcLot && ExistMQCJobDestLot)
			{
				...
				inheritSourceLotMQCJob(sourceLotData, destLotData, prodList, destProductList, destinationDurableName, eventInfo);
			}
			else
			{
		...
		if (sourceLotData.getProductQuantity() == 0)
		{
			...
			// Make Emptied
			sourceLotData.getUdfs().put("MERGEABLEFLAG", "");
			...
			ExtendedObjectProxy.getDummyProductReserveService().deleteDummyProductReserve(eventInfo, sourceLotData);
		}
	...
	public List<Product> remainProductList(List<Product> sourceList, List<Element> destinationList)
	{
		for (Product product : sourceList)
		{
			...
			{
				if (StringUtils.equals(product.getKey().getProductName(), elementDest.getChildText("PRODUCTNAME")))
				{
	...
	private List<ProductP> setproductPSequence(EventInfo eventInfo, List<Element> srcProductList, Lot srcLotData, Lot destLotData) throws CustomException
	{
		List<ProductP> productPSequence = new ArrayList<ProductP>();

		String offSet = getFirstOffSet(destLotData);
		...
		for (Element eleProduct : srcProductList)
		{
			...
			ExtendedObjectProxy.getDummyProductAssignService().createDummyProductAssign(eventInfo, productData, srcLotData, destLotData);
		}

		return productPSequence;
	}

	private String getFirstOffSet(Lot lotData) throws CustomException
	{
	...
	private void setProductRequestQuantity(EventInfo eventInfo, Lot srcLotData, Lot destLotData, int incrementQty) throws InvalidStateTransitionSignal, FrameworkErrorSignal, NotFoundSignal,
			DuplicateNameSignal, CustomException
	{
		// Decrease Source Quantity
		decrementQuantity(eventInfo, srcLotData, incrementQty);

		// Increase Dest WorkOrder Quantity
		incrementQuantity(eventInfo, destLotData, incrementQty);
	}

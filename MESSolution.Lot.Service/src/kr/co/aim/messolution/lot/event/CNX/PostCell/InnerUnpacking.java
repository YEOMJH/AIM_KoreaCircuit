...
public class InnerUnpacking extends SyncHandler {

	private static Log log = LogFactory.getLog(InnerUnpacking.class);
	
	@Override
	public Object doWorks(Document doc) throws CustomException
	{
		String innerBoxName = SMessageUtil.getBodyItemValue(doc, "PROCESSGROUPNAME", true);
		...
		List<Lot> lotList = MESLotServiceProxy.getLotServiceUtil().getLotListByProcessGroup(innerBoxName);
		...
		for (Lot lot : lotList)
		{
			...
			List<Map<String, Object>> operationData = MESLotServiceProxy.getLotServiceUtil().getOperationByDetailOperType(lot.getFactoryName(), lot.getProductSpecName(), lot.getProductSpecVersion(),
					lot.getProcessFlowName(), lot.getProcessFlowVersion(), "InnerPack");
		...
		ProcessGroup processGroup = MESProcessGroupServiceProxy.getProcessGroupServiceUtil().getProcessGroupData(innerBoxName);

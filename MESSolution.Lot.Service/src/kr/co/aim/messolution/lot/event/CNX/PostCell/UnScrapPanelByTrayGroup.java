...
public class UnScrapPanelByTrayGroup extends SyncHandler {

	private static Log log = LogFactory.getLog(UnScrapPanelByTrayGroup.class);
	
	@Override
	public Object doWorks(Document doc) throws CustomException {
		String trayGroupName = SMessageUtil.getBodyItemValue(doc, "DURABLENAME", true);
		String factoryName = SMessageUtil.getBodyItemValue(doc, "FACTORYNAME", 	false);

		List<Map<String, Object>> lotList = MESLotServiceProxy.getLotServiceUtil().getScrapLotListByTrayGroup(trayGroupName);
		ConstantMap constantMap = GenericServiceProxy.getConstantMap();
		EventInfo eventInfo = EventInfoUtil.makeEventInfo("UnScrapLotByTrayGroup", this.getEventUser(), this.getEventComment(), "", "");
		...
		for (Map<String, Object> lotMap : lotList) 
		{
			...
			Node nextNode = ProcessFlowServiceProxy.getNodeService().getNode(lotData.getFactoryName(), lotData.getProcessFlowName(), lotData.getProcessFlowVersion(),
					constantMap.Node_ProcessOperation, lotData.getUdfs().get("BEFOREOPERATIONNAME").toString(), lotData.getUdfs().get("BEFOREOPERATIONVER").toString());
			...
			lotUdf.put("BEFOREOPERATIONVER",
					oldLot.getProcessOperationVersion());
		...
		// change scrap qty
		MESWorkOrderServiceProxy.getProductRequestServiceImpl().ChangeScrapQty(eventInfo, productRequestName, 0, lotList.size());

		// Check WO State
		ProductRequest newProductRequestData = MESWorkOrderServiceProxy.getProductRequestServiceUtil().getProductRequestData(productRequestName);
				...
				(newProductRequestData.getPlanQuantity() > newProductRequestData.getFinishedQuantity() + newProductRequestData.getScrappedQuantity())) 
		{
			...
			MESWorkOrderServiceProxy.getProductRequestServiceImpl().makeReleased(newEventInfo, productRequestName);
		}

...
public class SplitLot extends SyncHandler {
	...
	public Object doWorks(Document doc) throws CustomException
	{
		...
		if ((StringUtils.equals(lotData.getFactoryName(), "ARRAY")||StringUtils.equals(lotData.getFactoryName(), "TP")||StringUtils.equals(lotData.getFactoryName(), "OLED")) 
				&& (lotData.getProductionType().equals("M")||lotData.getProductionType().equals("D")))
		{
			if (!productSpecData.getUdfs().get("PROCESSFLOWTYPE").equals("TP-DUMMY")) 
			{
				// Only can Split at Prepare Flow && no MQCPlan
		...
		for (Element eleDurable : eleDurableList)
		{
			...
			for (Element eleProduct : eleProductList)
			{
				...
				if (StringUtils.equals(productData.getFactoryName(), "TP"))
				{
			...
			// Split Lot
			TransferProductsToLotInfo transitionInfo = 
					MESLotServiceProxy.getLotInfoUtil().transferProductsToLotInfo(destLotData.getKey().getLotName(), eleProductList.size(), productPSequence, lotData.getUdfs(), new HashMap<String, String>());

...
public class MergeLotForIFI extends SyncHandler {
	...
	public Object doWorks(Document doc) throws CustomException
	{
		...
		String destinationDurableName = SMessageUtil.getBodyItemValue(doc, "DEST_DURABLENAME", true);
		...
		List<Element> eleDestiProductList = SMessageUtil.getSubSequenceItemList(eleBody, "DEST_PRODUCTLIST", true);
		...
		List<ProductP> productPSequence = setproductPSequence(eventInfo, srcProductList, destinationDurableName);
		...
		// Merge Lot
		TransferProductsToLotInfo transitionInfo = MESLotServiceProxy.getLotInfoUtil().transferProductsToLotInfo(destLotData.getKey().getLotName(), srcProductList.size(), productPSequence,
				destLotData.getUdfs(), new HashMap<String, String>());
	...
	private List<ProductP> setproductPSequence(EventInfo eventInfo, List<Element> srcProductList, String destDurableName) throws CustomException
	{
		...
		for (Element eleProduct : srcProductList)
		{
			...
			ExtendedObjectProxy.getOriginalProductInfoService().deleteOriginalProductInfo(eventInfo, productName);
		}
